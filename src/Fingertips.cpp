...
void Fingertips::drawContours()
{
    if ( initialized ) {
        for ( int i=0; i<contours.size(); i++ ) {
            ...
            // int last = hull.size();
            // for ( int j=0; j<last; j++ ) {
            //     // cv::circle( field, hull[j], 10, Scalar( 1.0f ) );
            //     ci::Vec2f jnow( curve[hull[j]].x, curve[hull[j]].y );
            //     ci::Vec2f jprev( curve[hull[(j-1) % last]].x, curve[hull[(j-1) % last]].y );

            //     gl::drawSolidCircle( jnow, 10.0f );
            //     gl::drawLine( jnow, jprev );
            //     // cv::line( field, hull[j], hull[j-1], Scalar( 1.0f ) );
            // }

            // draw curve hull
            // for ( int j=0; j<hull.size(); j++ ) {
            //     cv::circle( field, curve[hull[j]], 10, Scalar( 1.0f ), 3 );
            //     if( j == 0 ) {
            //         cv::line( field, curve[hull[j]], curve[hull[hull.size()-1]], Scalar( 1.0f ) );
            //     } else {
            //         cv::line( field, curve[hull[j]], curve[hull[j-1]], Scalar( 1.0f ) );
            //     }
            // }
        }
    }
